#!/bin/bash
# Quick fix: Create static index.html and configure nginx to serve it
# This is the fastest way to get the frontend working without rebuilding

set -e

echo "🚀 Quick fix: Creating frontend without rebuilding op-web"
echo ""
...
# Create index.html
cat << 'HTMLEOF' | sudo tee "$STATIC_DIR/index.html" > /dev/null
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    ...
    <main>
        ...
        <div class="input-area">
            <div class="input-wrapper">
                <textarea id="input" rows="2" placeholder="Ask me anything... (Enter to send, Shift+Enter for new line)"></textarea>
    ...
    <script>
        ...
        function addMessage(content, role, toolsExecuted = []) {
            ...
            if (toolsExecuted && toolsExecuted.length > 0) {
                ...
                toolsDiv.innerHTML = '🔧 Tools: ' + toolsExecuted.map(t => 
        ...
        addMessage('👋 Welcome to OP-DBUS Chat! I can help you manage your Linux system using D-Bus, OVS, rtnetlink, and more. What would you like to do?', 'assistant');
    </script>
</body>
</html>
HTMLEOF
...
# Update nginx config to serve static files at / and proxy API calls
...
# Enable site
sudo ln -sf "$NGINX_CONF" /etc/nginx/sites-enabled/op-dbus.conf
...
# Test
echo "  / -> $(curl -s -o /dev/null -w '%{http_code}' https://op-dbus.ghostbridge.tech/)"
echo "  /api/health -> $(curl -s -o /dev/null -w '%{http_code}' https://op-dbus.ghostbridge.tech/api/health)"
